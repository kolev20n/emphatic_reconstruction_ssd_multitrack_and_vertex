...
namespace ROOT
{
  namespace Minuit2
  {
    double my_straight_line_fcn::operator()(const vector<double>& parameters) const
    {
      ...
      {
        shortest_distance_vector = cluster_lines[i].line_direction.Cross(track_direction);
        dummy_vector.SetCoordinates(parameters[0], parameters[1], the_midpoint_z);
        
        dummy_double = fabs(shortest_distance_vector.Dot(cluster_lines[i].line_center - dummy_vector)) / sqrt(shortest_distance_vector.Mag2());
