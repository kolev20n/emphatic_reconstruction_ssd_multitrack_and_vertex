...
bool cluster_manager::cluster(vector<hit_pair> hit_pair_list[], vector<double> clusters[], int actual_number_of_plates, string cluster_interpretation_method)
{
  // assume there are no more than 800 hits per plate
  const int max_hits_per_plate = 800;
  int cluster_multiplicity[max_number_of_plates][max_hits_per_plate];
  // theoretically there can be as many clusters in the plane as hits
  int clusters_per_plane[max_number_of_plates][max_hits_per_plate];
  bool single_cluster_in_each_plane[max_number_of_plates];
  vector<hit_pair> dummy_hit_pair_vector;
  vector<vector<hit_pair>> temp_clusters[max_number_of_plates];
  ...
  for (int i = 0; i < max_number_of_plates; i++)
  {
    ...
    for (int j = 0; j < max_hits_per_plate; j++)
    {
  ...
  for (int i = 0; i < actual_number_of_plates; i++)
  {
    ...
    if (hit_pair_list[i].size() >= max_hits_per_plate)
    {
      cout << "More than maximum assumed hits on a plate: " << hit_pair_list[i].size() << " is more than " << max_hits_per_plate << endl;
